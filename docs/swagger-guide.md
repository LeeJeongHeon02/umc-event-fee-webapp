...
- `GET /me`는 본인의 전화번호(`phoneNumber`, 미등록 시 null)를 포함한다. 비밀번호·해시는 노출하지 않는다.
- `POST /auth/logout`은 현재 웹앱 세션과 CSRF 쿠키를 무효화하고 본문 없는 `204`를 반환한다. 로그인 방식이나 승인 여부와 무관하게 사용한다.
- Swagger에서 로그아웃을 실행한 뒤 다른 보호 API를 호출하려면 다시 로그인해야 한다. 카카오 연결 해제는 수행하지 않는다.
