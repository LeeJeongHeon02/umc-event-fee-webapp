...
### 2026-09-04 마이페이지·로그아웃 검증 (`dev`, 로컬 변경)

- 백엔드: 49개 통과, 실패·스킵 없음. PostgreSQL Testcontainers 포함.
- 프론트엔드: 12개 파일의 18개 테스트와 타입 검사 통과. 카카오·로컬 정보 표시, 승인 대기 접근, 비로그인 차단, 로그아웃 성공·실패·세션 만료, 개인 데이터 캐시 삭제 검증.
- 운영 보안 체인: CSRF 없는 로그아웃 차단 및 기존 세션 보존, 로컬·카카오 세션 무효화, 쿠키 삭제 경로, 로그아웃 후 인증 거부, GET 로그아웃 거부 검증.
- 실제 API E2E 2개 통과. 기존 행사·납부 흐름과 로컬 가입 → 온보딩 → 모바일 마이페이지 → 로그아웃 → 재로그인 확인.
- 320px·390px에서 가로 넘침 없음, 390px 스크린샷으로 레이아웃 확인.
- Node 24.19.0 프로덕션 빌드 통과(`dist-check` 별도 출력). 커밋·푸시·배포는 수행하지 않음.

개발 `dev` 프로필은 기존의 고정 회원 인증 우회를 유지한다. 운영 환경의 로그아웃 후 접근 차단은 `prod` 보안 체인 통합 테스트로 별도 검증했다. DB 마이그레이션이나 추가 환경 변수는 필요하지 않다.
