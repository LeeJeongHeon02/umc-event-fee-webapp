...
- 로컬 아이디·비밀번호·전화번호 회원가입, BCrypt 검증과 세션 로그인, 기존 온보딩 연결
- 홈의 다가오는 행사 섹션을 활동 요약 대시보드 위로 이동
...
### 2026-09-04 마이페이지·로그아웃 구현

- `/mypage`에 내 정보, 로그인 방식, 본인 전화번호, 권한·회원 상태 및 활동 메뉴 제공.
- 모바일 하단 `마이`, 회원·운영진 상단 프로필에서 접근. 승인 대기 화면과 온보딩에서도 로그아웃 제공.
- `POST /auth/logout`: 카카오·로컬 공통 세션 무효화, 세션·CSRF 쿠키 삭제. CSRF 보호 유지.
- 성공 시 진행 중인 조회 취소 및 QueryClient 캐시 정리 후 로그인 화면 이동. 실패 시 재시도 안내.
- 개인정보 수정·계정 탈퇴·카카오 자체 로그아웃은 이번 범위에서 제외. 환경 변수 및 DB 마이그레이션 추가 없음.
