...
export function AdminMembersPage() {
  ...
  return <div className="admin-page">
    ...
    <section className="admin-list-panel"><div className="admin-table-wrap"><table className="admin-table">
      <thead><tr><th>회원</th><th>가입 계정</th><th>상태</th><th>역할</th><th>관리</th></tr></thead>
      <tbody>{members.data!.map((member) => <tr key={member.id}>
        <td><div className="member-cell"><span>{(member.name ?? member.kakaoProfileName ?? member.loginId ?? '회').slice(0, 1)}</span><div><strong>{member.displayNickname ?? member.loginId ?? '온보딩 전 회원'}</strong><small>{member.part?.replace('_', ' ') ?? '온보딩 전'}</small></div></div></td>
        <td>{member.kakaoProfileName ? `카카오 · ${member.kakaoProfileName}` : `로컬 · ${member.loginId}`}</td><td>{member.status}</td>
