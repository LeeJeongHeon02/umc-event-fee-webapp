...
export function AdminPaymentReportsPage() {
  ...
  return <div className="admin-page">
    ...
    <section className="admin-list-panel">
      <div className="admin-toolbar"><h2>확인 대기 목록</h2><label><span className="sr-only">송금 신고 검색</span>
        <input className="text-input" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="부원, 입금자 또는 행사·회비명 검색" /></label></div>
      <div className="admin-table-wrap"><table className="admin-table">
        ...
        <tbody>{filtered.map((item) => <tr key={item.paymentId}>
          <td>{item.nickname}</td>
          <td>{item.source ? <Link to={item.source.type === 'EVENT'
          ...
          <td>{formatWon(item.amount)}</td><td><StatusBadge status={item.status} /></td>
          <td><AdminReviewActions memberName={item.name} paymentId={item.paymentId} version={item.version} /></td>
        </tr>)}</tbody>
      </table></div>
