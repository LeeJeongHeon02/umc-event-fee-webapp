    ...
    void 현재_구현된_API의_Swagger_OpenAPI_명세를_제공한다() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(jsonPath("$.paths['/admin/events/{eventId}/participants'].get.summary")
                        .value("행사별 참가자·납부 현황"))
                .andExpect(jsonPath("$.paths['/auth/local/register'].post.summary").value("로컬 계정 회원가입"))
                .andExpect(jsonPath("$.paths['/auth/local/login'].post.summary").value("로컬 계정 로그인"))
                .andExpect(jsonPath("$.paths['/auth/local/login'].post.security").isArray())
